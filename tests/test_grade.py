...
class TestGrade:
    def test_grade_create(self, universe_service, student_response, teacher_response):
        ...
        check.equal(
            grade_response.grade, grade_value, f"Wrong teacher id. AR: '{grade_value}', ER: '{grade_response.grade}'"
        )
    ...
    def test_get_grade(self, universe_service, grade_response):
        ...
        assert any(grade_response.model_dump().items() <= item.model_dump().items() for item in get_grade_response), (
            f"Get request return wrong grade, \
    ...
    def test_get_grade_stats_group(self, universe_service, group_student_teacher_factory, grade_factory):
        ...
        grade_factory(
            teacher_id=group_student_teacher_1["teacher"].id, student_id=group_student_teacher_1["student"].id
        )
        count = 3
        multi_grades = [
            grade_factory(
                teacher_id=group_student_teacher_1["teacher"].id, student_id=group_student_teacher_2["student"].id
            )
    ...
    def test_get_grade_stats_teacher(self, universe_service, group_student_teacher_factory, grade_factory):
        ...
        multi_grades = [
            grade_factory(
                teacher_id=group_student_teacher_2["teacher"].id, student_id=group_student_teacher_1["student"].id
            )
            for _ in range(count)
        ]
        expected_grade = grade_stats_calculate(multi_grades=multi_grades)
